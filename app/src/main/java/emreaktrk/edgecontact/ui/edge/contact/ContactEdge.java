...
public final class ContactEdge extends Edge implements ContactAdapter.IDelegate {
  ...
  private void unregisterEvent() {
    if (getContext() != null) {
      LocalBroadcastManager.getInstance(getContext()).unregisterReceiver(mEvent);
    }
  }

  @Override
  public void onCallClicked(final Contact contact) {
    PermissionHelper.call(
        getActivity(),
        new PermissionHelper.Callback() {
          @Override
          public void onRationale() {
  ...
  public void onAddClicked() {
    ...
    intent.setDataAndType(
        ContactsContract.Contacts.CONTENT_URI, ContactsContract.CommonDataKinds.Phone.CONTENT_TYPE);
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    final Contact contact =
        ContactResolver.from(getContext())
            .setUri(data.getData())
    ...
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N_MR1) {
      ShortcutInfo shortcut =
          new ShortcutInfo.Builder(getContext(), contact.getId())
